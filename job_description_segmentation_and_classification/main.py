...
# 分類的欄位
classify_column = ['company_description', 'people_need', 'job_duty', 'job_requirement', 'welfare']
classification = dict()
...
for col in classify_column:
    classification[col] = process_list_of_lists_neighbor(result, col, classify_column + ["none"])
...
trainer_ = Trainer(
    model=model, args=training_args, train_dataset=integer_labels_dataset_['train'], eval_dataset=integer_labels_dataset_['test']
)
...
for col in classify_column:
    result[col] = m.infer(list(job_parsed[col].astype("str")))
for col in classify_column:
    classification[col] = process_list_of_lists_neighbor_trans(result, col, classify_column + ["none"])
...
for col in classify_column:
    result[col] = m.infer(list(job_parsed[col].astype("str")))
for col in classify_column:
    classification[col] = process_list_of_lists_neighbor_trans_create_people(result, col, classify_column + ["none"])
